...
int32 Connector::handle_timeout() {
	_sock = sokcet(AF_INET, SOCK_STREAM, 0);
	if (_sock == -1) return -1;
...
int32 Connector::connect() {
	...
	int32 ret = ::connect(_sock, &addr, &addr_len);
	if (ret == 0) {
		// new handler.
		if (_timer_id > 0) {
		...
		return reactor()->register_handle(handler, IHandler::HET_Read | IHandler::HET_Write | IHandler::HET_Except);
	} else if (ret == -1) {
